...
IbdMerger::IbdMerger(const char *in_fn, const char *out_fn, const char *out_mode,
    ...
    : max_rec_allowed_by_ram(max_rec_allowed_by_ram), out_mode(out_mode), max_cm(max_cm),
      max_snp(max_snp)
{
...
IbdMerger::merge()
{
    ...
    do {
        ...
        auto it = std::remove_if(
            vec.begin(), vec.begin() + last, [](auto x) -> bool { return x.sid1 == 0; });
...
size_t
IbdMerger::find_group_start(size_t id)
{
    // size_t id_o = id;
...
bool
IbdMerger::is_mergeable(size_t id1, size_t id2)
{
...
void
IbdMerger::merge_range(size_t first, size_t last)
{
    auto &vec = in.get_vec();
