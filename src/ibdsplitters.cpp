...
IbdSplitter::IbdSplitter(const char *in_fn, const char *out_fn_prefix,
    const char *meta_fn, std::vector<region_label_t> &labels_, uint32_t max_label,
    float cm_threshold_, size_t in_ibdrec_vec_capacity, size_t out_ibdrec_vec_capacity)
...
void
IbdSplitter::split_ibd_rec(ibd_rec1_t rec)
{
    using namespace std;
    ...
    //
    auto cmp_start_id
        = [](region_label_t x, region_label_t y) -> bool { return x.pid_s < y.pid_s; };
...
void
IbdSplitter::transfer(ibd_rec1_t rec, uint32_t rid)
{
