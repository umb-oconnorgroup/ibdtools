...
IbdMatrix::IbdMatrix(std::vector<uint16_t> &in_cm10x_vec, uint8_t in_is_hap_pair)
{
    ...
    std::copy(in_cm10x_vec.begin(), in_cm10x_vec.end(), std::back_inserter(cm10x_vec));
}
...
void
IbdMatrix::calculate_total_from_ibdfile(IbdFile &ibdfile, bool use_hap_pair,
    const char *subpop_fn, float min_cM, float max_cM)
{
    if (max_cM < min_cM) {
...
void
IbdMatrix::get_histogram(std::vector<size_t> &count_vec, uint16_t win_size_in_10th_cm,
    MetaFile *meta, const char *subpop_fn, bool use_hap_pair)
{
    auto max_element = std::max_element(cm10x_vec.begin(), cm10x_vec.end());
    uint16_t max_val = *max_element;
    size_t sz
        = max_val / win_size_in_10th_cm + ((max_val % win_size_in_10th_cm) ? 1 : 0) + 1;
...
void
IbdMatrix::subset_matrix(const std::vector<uint32_t> &in_subpop_ids)
{
...
void
IbdMatrix::filter_matrix(uint16_t low, uint16_t upper)
{
...
void
IbdMatrix::print_subpop_ids_to_ostream(std::ostream &os)
{
