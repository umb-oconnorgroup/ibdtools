...
struct Node *nodeArr = NULL; // nodeArr for storing parsed data for each tree; reset
                             // before each tree parsing
int numNode = 0; // number of nodes needed; it is the sam for all trees in a single msms
                 // file, thus only needed intialize once.
...
// each point represent current ibd info for the sample pair (i, j), i is row no., j is
// col no.
long *ibdStartMatrix = NULL;   // ibd start coord
long *ibdEndMatrix = NULL;     // ibd end coord; it is actuall the next base of the ibd's
                               // last base, ie end coord not included in the segment
...
/* some constants */
double delta = 0; // tolerance when comparing ibd tmrca with segment tmrca. initialized
...
long min_bp
    = 0; // cutoff, min_bp is calcuated in main function, min_bp = min_cm * bp_per_cm
 ...
 * The critcal condition is that # of right parentheses is 1 more than # of left
 * parentheses between the comma separator and colon separator.
 */
int
parse_a_node(char *start, char *end, int *pCount, int parent)
{
    /*
    char *p;
 ...
 *
 * 	Key algorithm:
 * 		use a loop to track down the left child node of each left child node until reaching
 * the present sample the total edge length along the path is the time of the ancestral
 * node
 * */
void
parse_a_tree(char *tree, long tree_str_len)
{
    // intialize
    int count = 0;
    ...
    for (i = 0; i < numNode; i++) {
        if (nodeArr[i].left == -1)
            continue;
        t = 0.0;
        for (j = nodeArr[i].left; j != -1; j = nodeArr[j].left)
            t += nodeArr[j].len_above;
        nodeArr[i].time = t;
    }
    /* testing
...
/*
 * A recursive function to find all leaf nodes of the right arm of a ancestral node
 * ancNode (set LEFT) as well as all leaf nodes of the left arm of the ancestral node
 * (set RIGHT).
 *
 * Key algorithm:
 ...
 * */
void
find_present_sample(int index, int *array, int *pCount1)
{
 ...
 *
 * 	Key algorithm:
 * 	1) for each pair of present sample, from find_present_sample, obtain id1, id2, and
 * trmca 2) for each tree, we get the segStart, segEnd, info. 3) if ibdTmrcaMatrix[id1,
 * id2] == trmca, elongate this IBD by setting ibdEndMatrix[id1, id2] = segEnd 4)
 * otherwise, IBD is not continous with the segment being analysed. a) if the IBD is
 * longer than > 2cM right the IBD to stdout b) set IBD[id1, id2] to match current
 * segment info
 * */
void
...
main(int argc, char *argv[])
{
    ...
    if (argc < 6) {
        printf(
            " Usage. 	macs 50 10000 -t 0.0004 -r 0.0004 -h 1000 -T 2>/dev/null | \\\n"
            "		macs_tree_parser <chromN> <bp_per_cm> <time_scale_factor> "
            "<true_IBD_sampling_window> [<hom_het:0>]\n"
            "\n"
            "	chromN:                                            from 1 to 14 for pf; "
            "from 1 to 30 for human\n"
            "	bp_per_cm:                                         1000000 for human; "
            "15000 for p.f.\n"
            "	time_scale_factor:                                 4N0 (macs)\n"
            "	true_IBD_sampling_window:                          1 check all trees, "
            "1000, will check tree covers 0, 1000, 2000 \n"
            "		                                           suggestions: "
            "human->10000, pf->150; \n"
    ...
    delta
        = 0.5
          / time_scale_factor; // tolerance when comparing ibd tmrca with segment tmrca.
    char filename[10];
    sprintf(filename, "%d.ibd", chrom);
    f_ibd = fopen(filename, "w");
    sprintf(filename, "%d.log", chrom);
    f_log = fopen(filename, "w");
    sprintf(filename, "%d.map", chrom);
    f_map = fopen(filename, "w");
    sprintf(filename, "%d.vcf", chrom);
    f_vcf = fopen(filename, "w");
    ...
    while (getline(&buff, &lineSize, stdin) >= 0) {
        ...
        // check if segsites line
            ...
            if (position <= pre_position)
                position = pre_position + 1;

            // map file ( will not print every segsite, only print one position every
            // 0.01cM
            if (position > last_print_pos + bp_per_cm / 100) {
                fprintf(f_map, "%d\tsnp_%d_%ld\t%lf\t%ld\n", chrom, chrom, position,
                    position * 1.0 / bp_per_cm, position);
            ...
            if (segno == 0) {
                ...
                //       ## sample names
                if (hom_het == 0) // het
                    for (long i = 0; i < nsam / 2; i++)
                        fprintf(f_vcf, "\t%ld", i);
                else // hom
                    for (long i = 0; i < nsam; i++)
                        fprintf(f_vcf, "\t%ld", i);
            ...
            fprintf(f_vcf, "%d\t%ld\tsnp_%d_%ld\t%c\t%c\t.\t.\tPR\tGT", chrom, position,
                chrom, position, ATGC[ref], ATGC[alt]);
            //	printf allele columns
            if (hom_het == 0) // het
                for (int col = 0; col < nsam; col += 2)
                    fprintf(f_vcf, "\t%c|%c", pChar[col], pChar[col + 1]);
            else // homo
                for (int col = 0; col < nsam; ++col)
                    fprintf(f_vcf, "\t%c|%c", pChar[col], pChar[col]);
            fprintf(f_vcf, "\n");
            ++segno;
        }
        // --------------
        // for lines with tree info
            ...
            // Get seg length
            for (pChar = buff; *pChar != '['; ++pChar) {
            };
            ++pChar; // move to seg length position
            segLength = strtol(pChar, NULL, 10);
            segStart = segEnd;
            segEnd += segLength;

            // if a mulplte of true_IBD_sampling_window is not on the segment, skip
            // parsing the tree using browning method
            if (segEnd / true_IBD_sampling_window
                == segStart / true_IBD_sampling_window) {
                continue;
            }

            for (tree = pChar; *tree != '('; tree++) {
            }; // find the first (
