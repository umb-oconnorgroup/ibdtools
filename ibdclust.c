...
static inline int upper_trangle_matrix_row_col_to_array_index(int id1, int id2,
                                                              int num_samples) {
  int temp;
  if (id1 > id2) {
    temp = id1;
...
void array_index_to_upper_trangle_matrix_row_col(int index, int num_samples,
                                                 int *pid1, int *pid2) {
  int row, col, row_len;
...
void clust_init_from_binary(struct CLUST *self, const char *filename) {
  struct IBDTOTAL {
  ...
  self->nodes =
      (struct NODE *)calloc((2 * num_samples - 1), sizeof(struct NODE));
  ...
  for (int col = 0; col < num_samples; col++) {
    ...
    for (int row = 0; row < num_samples; row++) {
      if (row == col)
        continue;
      index =
          upper_trangle_matrix_row_col_to_array_index(row, col, num_samples);
      value = cM[index];
      if (value < min) {
        min = value;
        row_min = row;
...
void clust_init(struct CLUST *self, const char *sample_file,
                const char *data_file) {
  FILE *fp;
  char buff[1000], *p = NULL;
  size_t size = 1000;
  size_t counter = 0;
  size_t num_pairs = 0;
...
void clust_print_info(struct CLUST *self) {
  int index = 0;
  ...
  for (size_t i = 0; i < self->num_samples; i++) {
    for (size_t j = 0; j < self->num_samples; j++) {
      if (i == j)
        ...
        index = upper_trangle_matrix_row_col_to_array_index(i, j,
                                                            self->num_samples);
...
int clust_find_min_cM_index(struct CLUST *self, int *prow, int *pcol) {
  ...
  *pcol = min_index;
  ...
  return upper_trangle_matrix_row_col_to_array_index(*prow, *pcol,
                                                     num_run_samples);
}

void clust_update_nodes_before_clustering(struct CLUST *self, int row,
                                          int col) {
  ...
  index =
      upper_trangle_matrix_row_col_to_array_index(row, col, self->num_samples);
  ...
  fprintf(stderr,
          "child1- height: %lf, len_to_parent: %lf, left: %d, label: %d\n",
...
void clust_update_matrix_after_clustering(struct CLUST *self, int row,
                                          int col) {
  int temp;
  int index1, index2;
  int weight1, weight2;
  ...
  if (col != last) {
    for (size_t i = 0; i < num_run_samples; i++) {
      ...
      index2 =
          upper_trangle_matrix_row_col_to_array_index(i, last, num_samples);
      cM[index1] = cM[index2];
      cM[index2] = -1;
    }
    /*update samples labels*/
  ...
  for (int i_col = 0; i_col < num_run_samples; i_col++) {
    ...
    if (index == col || index == row || i_col == row) {
      ...
      for (int j_row = 0; j_row < num_run_samples; j_row++) {
        ...
        int ind = upper_trangle_matrix_row_col_to_array_index(j_row, i_col,
                                                              num_samples);
        value = cM[ind];
        if (value < min) {
          min = value;
 ...
 */
void clust_print_newick_tree(struct CLUST *self) {
...
int test_binary(const char *fn_matrix_binary) {
  struct CLUST myclust;
...
int main(int argc, char *argv[]) {
  ...
  if(argc!=2)
  {
	  fprintf(stderr, "\n\nUsage: ./ibdclust <binary_matrix_file> 2>/dev/null \n\n"
			  "stdout will output newick tree(1st line) and sample order after clusterization (2nd line)\n\n ");
	  exit(1);
  }
